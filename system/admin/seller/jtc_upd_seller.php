...
if($name==""){
  ...
  $sql_select_sellers = "SELECT * FROM sellers WHERE email='".$email."' AND id<>'".$id."'";
  ...
  if($sql_select_sellers_prepare->rowCount()==0){
    $sql_select_sellers = "SELECT * FROM sellers WHERE phone='".$phone."' AND id<>'".$id."'";
    $sql_select_sellers_prepare = $dbconnection->prepare($sql_select_sellers);
    $sql_select_sellers_prepare->execute();
    if($sql_select_sellers_prepare->rowCount()==0){
      $info = array(
      ...
      $result = alterar("sellers", $info, "id=".$id);
      if($result){
        $msgid = 14;
        $data = "Vendedor";
...
header("Location: ?folder=seller/&file=jtc_fminsupd_seller&ext=php&updid=".$id."&msgid=".$msgid."&data=".$data);